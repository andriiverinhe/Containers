...
TEST(MultisetTest, FindMethod) {
  ...
  auto it1 = test_set.find(3);
  auto it2 = test_set.find(2);
  auto it3 = test_set.find(3);
  auto it4 = test_set.find(4);

  // std::cout << (it2 != (test_set.end())) << std::endl;
  EXPECT_TRUE(it1 == it1);
  EXPECT_TRUE(it2 != (test_set.end()));
  EXPECT_TRUE(it3 != test_set.end());
  EXPECT_EQ(it4, test_set.end());
}

TEST(MultisetTest, insertMany) {
  s21::multiset<int> my_set = {1, 2, 3, 4, 56};
  my_set.insert_many(12, 3, 4, 56, 6, 2);
}
